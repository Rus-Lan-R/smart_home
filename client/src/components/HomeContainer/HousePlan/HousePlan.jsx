...
export default function SimpleContainer() {
	...
	const handleClick = (e) => {
		...
		const styleMarker = {
			position: "relative",
			color: "black",
			left: `${event_offsetX}px`,
			top: `${event_offsetY}px`,
			width: 0,
			height: 0,
			visibility: "visible",
		};
	...
	return (
		<React.Fragment>
			<div
				style={{ borderTop: "2px solid #fff ", marginTop: 30, marginLeft: 20, marginRight: 20 }}
			></div>
			<CssBaseline />
			<MarkerButtons />
				...
				{rooms.items.map((item) => {
					...
					return (
						<div
							...
							key={item._id}
							style={styleMarker}
							onDoubleClick={(e) => handleDblCLick(e, item._id)}
						>
							{item.room}
							<IconPickerItem icon={`${item.picture}`} size={36} color="yellow" />
							<div className={classes.bubble}>
								{[...sensors]
									.filter((el) => el.room === item._id)
									.map((el) => {
										return (
											<div>
												{el.sensorName}: {el.value}{" "}
											</div>
										);
									})}
							</div>
