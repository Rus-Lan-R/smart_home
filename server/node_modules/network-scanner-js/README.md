...
### Web Portal
```javascript
const hosts = ['192.168.4.10', '192.168.4.18', '192.168.4.34', '192.168.4.68', '192.168.4.93','192.168.4.11']
netScan.monitorCluster(hosts)
```
This will open a web browser UI that will show latency stats for each host
![Screenshot](https://network-scanner-js-images.s3-ap-southeast-2.amazonaws.com/web-content.PNG "Web Monitor")
...
### SSH Client
```javascript
netScan.sshCommands = ['hostname', 'ls']
const config = { host:'test.rebex.net', port:22, user:'demo', password:'password' }
//To include a private key just include it in you config object as privateKey: require('fs').readFileSync('/path/to/private/key/id_rsa')
